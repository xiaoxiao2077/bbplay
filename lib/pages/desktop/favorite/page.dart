...
class _FavoritePageState extends State<DesktopFavoritePage> {
  List<VideoItem> favoriteList = [];
  ...
  int get selectingCount => favoriteList.where((item) => item.checked).length;
  ...
  Future<void> _fetchData({int page = 1}) async {
    ...
    try {
      ...
      final results = await Future.wait([
        FavoriteService.search(keyword, offset: offset, limit: 100),
        FavoriteService.searchCount(keyword),
      ]);
      ...
      final List<VideoItem> items =
          dataList.map((item) => VideoItem.fromFavorite(item)).toList();
  ...
  void _toggleSelectAll() {
    final shouldSelectAll = selectingCount != favoriteList.length;
    setState(() {
  ...
  Future<void> _deleteSelected() async {
    ...
    final bool confirmed = await showConfirmDialog(
      context,
      title: '确认删除',
      content: '确定要删除选中的$selectingCount个收藏吗？',
      confirmText: '删除',
      cancelText: '取消',
    );
    ...
    if (confirmed) {
      try {
        final selectedItems =
            favoriteList.where((item) => item.checked).toList();
        ...
        setState(() {
          favoriteList.removeWhere((item) => item.checked);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color.fromARGB(255, 225, 235, 229),
      appBar: AppBar(
        toolbarHeight: 1,
        elevation: 0,
            ...
            child: Row(
              children: [
                ...
                TextButton(
                  ...
                  child: Text(
                    selectingCount == favoriteList.length &&
